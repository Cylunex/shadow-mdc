...
- [mdcx-diy](https://github.com/cdlongbow/mdcx-diy)：来源优先级、字段合并和中文别名思路；未验证为探花专用来源。
- [AVScan](https://avscan.cc/about/)：仅作为用户主动使用的外部截图检索参考；没有公开 API，不自动上传本地画面。
...
- `.strm` 播放地址不是元数据详情页。扫描阶段不访问它，URI 凭据和查询参数也不进入持久化线索。
