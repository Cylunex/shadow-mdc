#!/usr/bin/env python3
"""Seed this week's top VR works (+ actors/posters) into the local catalog.

Not limited to JAV. Prefer running on the **box** (FANZA GraphQL + public VR
pages), then ``scripts/sync_catalog_to_nas.sh`` / ``--sync-nas``.

Does not replace curated yearly ``jav-vr-yearly-top``. Magnets save-only; no 115.
...
async def _run(arguments: argparse.Namespace) -> int:
    ...
    summary = {
        ...
        "failures": [
            {"key": f.key, "code": f.code, "title": f.title, "error": f.error}
...
def main() -> None:
    ...
    parser.add_argument("--no-log", action="store_true", help="do not write data/weekly-vr-runs/*.json")
