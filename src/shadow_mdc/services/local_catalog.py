...
_VIDEO_PART = re.compile(r"(?i)^v\s*(?:[\(\uFF08]\s*(\d+)\s*[\)\uFF09]|[-_. ]*(\d+))$")
_PREFIXED_PART = re.compile(
    r"(?i)^([pc])\s*(?:[\(\uFF08]\s*(\d+)\s*[\)\uFF09]|[-_. ]*(\d+))$"
)
...
def infer_media_category(*values: str) -> MediaCategory:
    joined = " ".join(unicodedata.normalize("NFKC", value).casefold() for value in values)
    if any(token in joined for token in ("国产", "國產", "china", "chinese")):
        return MediaCategory.CHINA
    if any(token in joined for token in ("韩国", "韓國", "korea", "korean")):
        return MediaCategory.KOREA
    if any(token in joined for token in ("欧美", "歐美", "europe", "western")):
        return MediaCategory.EUROPE
    if any(token in joined for token in ("日本", "japan", "jav")):
        return MediaCategory.JAPAN
...
def build_local_catalog_record(
    *,
    ...
    title = _local_title(path.stem, hints, subject)
    family = hints.family if hints.family is not ContentFamily.UNKNOWN else family_for_category(category)
    external_id = hashlib.sha256(
        f"{library_id}\0{relative.as_posix().casefold()}".encode()
    ).hexdigest()
    actors = hints.actors or ((subject,) if subject else ())
    series = hints.series
    if series is None and subject and _is_generic_file_name(path.stem):
        series = subject
...
def _local_title(raw_stem: str, hints: IdentityHints, subject: str | None) -> str:
    ...
    if suffix is not None:
        base = subject or hints.title or hints.code or "未命名影片"
...
def _is_generic_file_name(raw_stem: str) -> bool:
    normalized_stem = _remove_site_prefix(raw_stem)
    return (
        _generic_suffix(normalized_stem) is not None
        or _OPAQUE_FILE.match(normalized_stem) is not None
    )
