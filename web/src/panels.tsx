...
export function DisplayFilterBar(props: {
  query: string;
  ...
  return <>
    <div className="section-hero compact">
      <div>
        ...
        <h1>演员列表</h1>
        <p className="muted">共 {props.actors.length + props.nonJavActors.length} 位演员 · 标签：收藏 / 订阅 / 黑名单</p>
      </div>
    </div>
...
function JavActors({ actors, prefs, onActorTags, busy }: {
  ...
  const [tagFilter, setTagFilter] = useState<"all" | "favorite" | "subscribe" | "blacklist">("all");
  const [page, setPage] = useState(1);
  const deferredQuery = useDeferredValue(query);
  const normalizedQuery = deferredQuery.trim().toLocaleLowerCase();
  const tags = prefs?.actor_tags ?? {};
  const counts = useMemo(() => ({
    favorite: Object.values(tags).filter((item) => item.favorite).length,
    subscribe: Object.values(tags).filter((item) => item.subscribe).length,
    blacklist: Object.values(tags).filter((item) => item.blacklist).length
  }), [tags]);
  const visibleActors = useMemo(() => actors.filter((actor) => {
    const key = actor.id ?? actor.name;
    const tag = tags[key] ?? tags[actor.name];
    const tagMatches = tagFilter === "all"
      || (tagFilter === "favorite" && tag?.favorite)
      || (tagFilter === "subscribe" && tag?.subscribe)
      || (tagFilter === "blacklist" && tag?.blacklist);
    ...
    return tagMatches && categoryMatches && (!normalizedQuery || text.includes(normalizedQuery));
  }), [actors, category, normalizedQuery, tagFilter, tags]);
  useEffect(() => setPage(1), [category, deferredQuery, tagFilter]);
  ...
  return <>
    ...
    />
    <div className="tag-capsules" aria-label="标签筛选">
      <span>标签筛选:</span>
      {([
        ["all", "全部", actors.length],
        ["favorite", "收藏", counts.favorite],
        ["subscribe", "订阅", counts.subscribe],
        ["blacklist", "黑名单", counts.blacklist]
      ] as const).map(([id, label, count]) => (
        <button key={id} type="button" className={tagFilter === id ? "active" : "ghost"} onClick={() => setTagFilter(id)}>
          {label} · {count}
        </button>
      ))}
    </div>
        ...
        return (
          <div className="actor-tag-actions">
            ...
            <button type="button" className={tag.subscribe ? "active" : "ghost"} disabled={busy === `tag-${key}`}
              onClick={() => void onActorTags(key, { ...tag, subscribe: !tag.subscribe, blacklist: false })}>订阅</button>
            <button type="button" className={tag.blacklist ? "danger" : "ghost"} disabled={busy === `tag-${key}`}
              onClick={() => void onActorTags(key, { favorite: false, subscribe: false, blacklist: !tag.blacklist })}>黑名单</button>
          </div>
        );
      })()}
...
export function FilterWordsEditor() {
